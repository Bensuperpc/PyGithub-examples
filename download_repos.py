#!/bin/python
#
# download_repos.py - for download repos as tarball
#
...
for repo in g.get_user().get_repos():
    print('Download:' + repo.name)
    archive_link = repo.get_archive_link("tarball", repo.default_branch)
    r = requests.get(archive_link, allow_redirects=True)
    open(repo.name + '.tar.gz', 'wb').write(r.content)
    print('Download: OK')
